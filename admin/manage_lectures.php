...
// دالة مساعدة لجلب الأسماء بناءً على ID
        ...
        <table class="manage-table">
            ...
            <tbody id="lecturesTableBody">
                <?php if (empty($lectures_data)): ?>
                    <tr>
                        <td colspan="8" style="text-align: center;">لا توجد محاضرات لعرضها بعد.</td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($lectures_data as $lecture): ?>
                        <tr data-lecture-id="<?php echo htmlspecialchars($lecture['id']); ?>">
                            ...
                            <td><?php echo htmlspecialchars(getNameById($lecture['lecturer_id'], $lecturers_data)); ?></td>
                            <td><?php echo htmlspecialchars(getNameById($lecture['branch_id'], $branches_data)); ?></td>
                            <td><?php echo htmlspecialchars(getNameById($lecture['semester_id'], $semesters_data)); ?></td>
                            <td><?php echo htmlspecialchars(getNameById($lecture['group_id'], $groups_data)); ?></td>
                            <td><?php echo htmlspecialchars(getNameById($lecture['category_id'], $categories_data)); ?></td>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function displayLectures(lecturesToDisplay) {
                ...
                if (lecturesToDisplay.length === 0) {
                    lecturesTableBody.innerHTML = '<tr><td colspan="8" style="text-align: center;">لا توجد محاضرات مطابقة.</td></tr>';
                    return;
                }
            ...
            if (initialCategoryId) lectureCategoryFilter.value = initialCategoryId;
